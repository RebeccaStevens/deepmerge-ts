...
import { deepmergeCustom as deepmergeCustomImpl } from "../../lib/deno.js";
import type {
  deepmergeCustom as deepmergeCustomType,
  DeepMergeLeafURI,
...
const deepmergeCustom = deepmergeCustomImpl as typeof deepmergeCustomType;

Deno.test(
  "works just like non-customized version when no options passed",
  () => {
    const v = { first: true };
...
Deno.test("custom merge strings", () => {
...
Deno.test("custom merge arrays of records", () => {
  const x = {
    foo: [
      { bar: { baz: [{ qux: 35 }] } },
      { bar: { baz: [{ qux: 36 }] } },
...
Deno.test("custom merge records", () => {
  const x = {
    foo: {
      bar: 1,
    },
...
Deno.test("custom don't merge arrays", () => {
